...
all_hashes = defaultdict(list) # stores all hash to list (files and directories) mappings for checking duplicates
check_pattern = False # stores whether file names or directory names will be matched to a pattern
current_path = subprocess.check_output("pwd", shell = True).splitlines()[0] # stores the directory where program is running
dirs = [] # list of directories to traverse while searching for duplicates

# Parse the program arguments
parser = argparse.ArgumentParser()
action = parser.add_mutually_exclusive_group()
action.add_argument('-c', action = 'store', dest = 'command', help = 'Apply command to duplicates')
action.add_argument('-p', action = 'store_true', default = False, dest = 'print_duplicates', help = 'Print duplicates')
file_type = parser.add_mutually_exclusive_group()
file_type.add_argument('-f', action = 'store_true', default = False, dest = 'check_files', help = 'Look for duplicate files')
file_type.add_argument('-d', action = 'store_true', default = False, dest = 'check_dirs', help = 'Look for duplicate directories')
parser.add_argument(action = 'store', nargs = '*', dest = 'dirs', help = 'Pattern to match and directories to search')
...
if results.dirs:
    ...
    for path in results.dirs:
        if current_path in path:
            ...
            dirs.append(current_path + "/" + path) # convert relative paths to absolute paths
if not results.dirs:
    dirs.append(current_path) # use current path if there is no directories as arguments
...
def dfs(pathname):
    ...
    for filename in os.listdir(pathname):
        ...
        if os.path.isdir(newpath):
            ...
            hash = subprocess.check_output("shasum -a 256 " + "\"" + newpath + "\"", shell = True).split()[0]
...
for hash, files in all_hashes.iteritems():
    if len(files) > 1 and hash != hashlib.sha256("").hexdigest(): # for not printing empty files or directories
        duplicates = []
        for file in files:
